...
print(weights.super, 
      i = 1)
#> 
#> For imputation 1 and the `super` weighting method, the median weight value is 1.27 (SD = 9.77; range = 0-259).
...
#>                                          
#>        1145   1030    697     591     475
#>  all 49.812 56.124 65.552 179.544 259.273
#> 
#> - Weight statistics:
#> 
#>     Coef of Var   MAD Entropy # Zeros
#> all       3.746 0.958    1.13       0
#> 
#> - Effective Sample Sizes:
#> 
#>              Total
...
#> +-----------+-----------------------+----------------------------+
#> 
#> Table: Imbalanced Covariates Averaging Across Imputed Datasets using `cbps`
...
#> +-----------+-----------------------+----------------------------+
#> 
#> Table: Imbalanced Covariates Averaging Across Imputed Datasets using `glm`
...
#> +-----------+-----------------------+----------------------------+
#> 
#> Table: Imbalanced Covariates Averaging Across Imputed Datasets using `gbm`
...
#> +-----------+-----------------------+----------------------------+
#> 
#> Table: Imbalanced Covariates Averaging Across Imputed Datasets using `bart`
...
#> +-----------+-----------------------+----------------------------+
#> 
#> Table: Imbalanced Covariates Averaging Across Imputed Datasets using `super`
```
...
wide data (see Step 4). This can be helpful for including any
time-invariant confounders that could not be successfully balanced using
the weights.
...
#> +===================+============+============+===========+=============+===========+==============+
#> | (h-h-h) - (l-l-l) | 0.03035381 | 0.01948629 | 0.1991633 | -0.02526438 | 0.085972  | 0.1991633    |
#> +-------------------+------------+------------+-----------+-------------+-----------+--------------+
```
...
#> 1 (h-h-h) - (l-l-l) 0.03035381 0.01948629  1.557701 3.740082 0.1991633
#>      conf.low conf.high    conf.low conf.high  dose p.value_corr
#> 1 -0.02526438  0.085972 -0.02526438  0.085972 3 - 0    0.1991633
```
...
summary(results.s1, 
        save.out = save.out)
#>                term   estimate  std.error statistic       df    p.value
#> 1 (h-h-h) - (l-l-l) 0.03327779 0.01660091  2.004577 6.877882 0.08576757
#>       conf.low  conf.high     conf.low  conf.high  dose p.value_corr
#> 1 -0.006118846 0.07267443 -0.006118846 0.07267443 3 - 0   0.08576757
```
...
summary(results.s2, 
        save.out = save.out)
#>                term   estimate  std.error statistic       df   p.value
#> 1 (h-h-h) - (l-l-l) 0.02640764 0.02274001  1.161285 2.655187 0.3392188
#>      conf.low conf.high    conf.low conf.high  dose p.value_corr
#> 1 -0.05157968  0.104395 -0.05157968  0.104395 3 - 0    0.3392188
```
