...
# data=read.csv("/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/Tutorial paper/merged_tutorial.csv")
# data=data%>%dplyr::filter(TAge<60)
# data=data[data$TAge!=48,]
# data$EF_avg_perc= rowMeans(data[,54:60], na.rm=T)
# data$fscore=NULL
# data=data[,!grepl("pct", colnames(data))]
# # data$lnINR=log10(data$InRatioCor)
# # data$InRatioCor=NULL
# data[data=="NaN"]=NA
# write.csv(data, "/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/Tutorial paper/merged_tutorial_filtered.csv", row.names = F)

# temp=data%>%dplyr::select(InRatioCor, lnINR)
...
object <- msmObject(
  ...
  time_varying_variables= c("ADHDtcTot", "ALI_Le","SAAmylase", "EARS_TJo","Chaos_Unstable", "Chaos_HomEnv", "SEmply","MDI",  "LESMnPos","LESMnNeg","pcx_pos", "pcx_neg", "pcx_engaged", "pcx_sensitive", "pcx_CompTwo", "StrDif_ES", "StrDif_CP", "StrDif_HY", "StrDif_PP", "StrDif_PR", "StrDif_Tot", "RHasSO","QSSOverallMe", "WndNbrhood", "EF_avg_perc","CCTHrs","TcAge","CORTB","Cotin_rep1ngmL", "PEmply", "IBRApro", "IBRAfct", "IBRAttn","IBRReac", "IBRIrit","B18Raw", "ESETA1", "HOMEETA1", "CTSETA1","InRatioCor"),
  ...
  continuous_variables=c("PmAge2", "ADHDtcTot", "ALI_Le", "CCTHrs", "SAAmylase", "EARS_Tjo", "Chaos_Unstable", "Chaos_HomEnv", "CORTB", "Cotin_rep1ngmL", "IBRApro", "IBRAfct", "TcAge","IBRReac", "IBRAttn", "IBRIrit","LESMnPos", "LESMnNeg","B18Raw", "ESETA1", "HOMEETA1", "CTSETA1", "pcx_pos", "pcx_neg","pcx_engaged", "pcx_sensitive", "PmMrSt2",
                         "pcx_CompTwo", "StrDif_ES", "StrDif_CP", "StrDif_HY", "StrDif_PP", "StrDif_PR", "StrDif_Tot", "QSSOverallMe", "WndNbrhood", "InRatioCor", "EF_avg_perc"),
  
  #list any covariates that are factors here; default is none, or that all variables are continuous 
  factor_covariates=c("state","TcSex2", "TcBlac2", "PmBlac2",  "PEmply", "SEmply", "RHasSO"), #list of characters
  ...
  exposure=c("InRatioCor"), #list of strings
  ...
  reference="h-h-h", #optional: set a reference history of high ("h") and low ("l") levels of exposure separate by a "-"; the default is set to the history denoting low ("l") exposure at all time points; character string of "h" and "l" separated by a "-" 
  ...
  time_var_exclude=c("ADHDtcTot.6", "ADHDtcTot.15", "ADHDtcTot.24", "ADHDtcTot.35","ALI_Le.6", "ALI_Le.15","ALI_Le.24","ALI_Le.58","SAAmylase.35", "SAAmylase.58",
                     "EARS_TJo.6","EARS_TJo.15", "EARS_TJo.58",  "Chaos_HomEnv.6", "Chaos_HomEnv.15", "Chaos_HomEnv.24", "Chaos_Unstable.6", "Chaos_Unstable.15", "Chaos_Unstable.24",
                     "CORTB.35", "CORTB.58", "Cotin_rep1ngmL.35", "Cotin_rep1ngmL.58", "Cotin_rep1ngmL.35", "IBRApro.35", "IBRApro.58", "IBRAfct.35", "IBRAfct.58", 
                     "IBRAttn.35", "IBRAttn.58", "IBRReac.35", "IBRReac.58", "IBRIrit.35", "IBRIrit.58", "MDI.24", "MDI.35", "MDI.58", "LESMnPos.6", "LESMnPos.15", "LESMnPos.58", 
                     "LESMnNeg.6", "LESMnNeg.15", "LESMnNeg.58", "B18Raw.35",  "pcx_pos.58", "pcx_neg.58","pcx_engaged.58", "pcx_sensitive.58", "pcx_CompTwo.6", "pcx_CompTwo.15",
                     "pcx_CompTwo.24", "pcx_CompTwo.35", "StrDif_ES.6", "StrDif_ES.15","StrDif_ES.24", "StrDif_CP.6", "StrDif_CP.15","StrDif_CP.24", "StrDif_HY.6", 
                     "StrDif_HY.15","StrDif_HY.24", "StrDif_PP.6", "StrDif_PP.15","StrDif_PP.24", "StrDif_PR.6", "StrDif_PR.15","StrDif_PR.24", "StrDif_Tot.6", 
                     "StrDif_Tot.15","StrDif_Tot.24", "QSSOverallMe.15", "WndNbrhood.15", "EF_avg_perc.6", "EF_avg_perc.15", "EF_avg_perc.24"), #default set to none
...
# covariates_to_include <- identifyPotentialConfounds(object, all_potential_covariates)
...
#does pre balance checking on full forms to examine initial imbalance
preBalanceChecking(object, wide_long_datasets, full_forms)

#create short forms with time-varying covariates only at t-1 for initial weights creation
short_forms <- createShortForms(object, full_forms, keep=list(exposure=c("HOMEETA1"),
                                                         time=c(24),
                                                         tv_covar=c("InRatioCor.6")))
...
data_for_model_with_weights<- createWeights(object, wide_long_datasets, short_forms, read_in_from_file="no") 

#assesses balance for short forms
balance_stats <- assessBalance(object, short_forms, data_for_model_with_weights, weighted=1)
...
#assesses balance for full forms
balance_stats_full <- assessBalance(object, full_forms, data_for_model_with_weights, weighted=1)
...
#re-create weights, if needed ONLY
data_for_model_with_weights<- createWeights(object, wide_long_datasets, new_short_forms, read_in_from_file="no") 

```


### 6. Truncate weights to create one per person per exposure-outcome pair
...
all_models <- fitModel(object, data_for_model_with_weights_cutoff, unbalanced_covariates_for_models, model="m4")
