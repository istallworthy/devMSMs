...
object <- msmObject(
  ...
  exposures=c("InRatioCor", "HOMEETA1"), #list of strings
  ...
  weights_method="cbps", #method for calculating weights: cbps, npcbps, bart, ps
  ...
  outcomes=c("EF_avg_perc"), #list of strings 
...
covariates_to_include <- identifyPotentialConfounds(object, potential_covariates)
...
imp_data_w<- createWeights(object, wide_long_datasets, forms, read_in_from_file="yes") 
...
```{r}
#truncate weights 
data_for_model_with_weights_cutoff <- truncateWeights(object, data_for_model_with_weights, imp_data_w)
