...
author: "Isa Stallworthy", "Meriah DeJospeh", "Daniel Berry"
date: 10/27/2022"
output: html_document
---

**NOTE: now wondering if this text content should just go in the tutorial paper instead? 
...
[more collider resources? see david's stuff & ask dan] 


*Weight truncation/sensitivity analyses
...
The number of imputed datasets is specified as 'm' in the msm object. 
```{r}
...
The optional user parameter 'just_imputed' allows you to specify whether you just ran the imputed code ("yes"; default) or you'd rather read in the imputations saved locally from a previous run ("no"). 
```{r}
...
weights_models=createWeights(object, wide_long_datasets, forms, read_in_from_file="no",
                             ATT=0, iterations=1000, standardize=FALSE, method="exact", twostep=TRUE, sample.weights=NULL, baseline.forumula=NULL, diff.formula=NULL) #optional weights parameters (see CBPS documentation for more)
...
The user has the option to provide more formal labels for the exposure(s) ('exposure_labels') and outcome ('outcome_labels') to be used for plotting.  
```{r}
