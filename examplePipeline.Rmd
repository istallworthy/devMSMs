...
home_dir="/Users/isabella/Library/CloudStorage/Box-Box/BSL General/Isa/MSMs/"
...
MSMs focus on the covariates that are associated with both exposure and outcome and thus could act like a confounding variable. This code assumes that you have included all possible confounding variables as columns in your dataset. More specifically, a potential confound for a given exposure at a given time point is any covariate correlated with the exposure at that time point, lagged values of the exposure. 
The user can specify any covariates to exclude ('exclude_covariates') as well as those to include ('keep_covariates'), regardless of their correlation with exposures/outcomes.  Append ".x" to indicate a time-varying variable at a specific time point only. 
```{r}
...
```{r}
#any covariates you wawnt included (optional)
covariates=c("Timeofday_c")
...
makeMplusInputs(ID, home_dir, missing, exposures, exposure_epochs, covariates, reference, outcomes, hi_cutoff=.75, lo_cutoff=.25)
