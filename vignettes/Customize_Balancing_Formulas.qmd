...
obj = initMSM(
  data = sim_data_wide, exposure = exposure, tv_conf = tv_conf, ti_conf = ti_conf
)
```
<br> 
...
```{r}
full_formulas <- createFormulas(obj  = obj, type = "full", concur_conf = "B18Raw.24")
print(full_formulas)
```
...
```{r}
# custom <- list(
#   "full_form-6" = as.formula("ESETA1.6 ~ BioDadInHH2 + DrnkFreq + gov_assist"),
...
```{r}
# full_formulas <- createFormulas(
#   exposure = exposure, exposure_time_pts = exposure_time_pts, outcome = outcome,
#   type = "full", ti_conf = ti_conf, tv_conf = tv_conf,
...
```{r}
short_formulas <- createFormulas(obj = obj, type = "short", keep_conf = c("InRatioCor.6"))
