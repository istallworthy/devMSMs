...
Specifying at least time-varying exposures as time-varying confounders is required for the use of this package as required input for the `createFormulas()` *devMSMs* function (see *Workflows* vignettes). In `tv_conf` as a list, in quotations, provide the names of all variables in wide format (e.g., “variable.t”) in your dataset that are time-varying (including time-varying confounders, exposures, and outcomes). These should include all time-varying exposure variables and all outcome variables present in the dataset (e.g., “variable.t”). Note that time-varying confounders should also include any confounders measured repeatedly at all time points (e.g., InRatioCor) or any collected only at one or several specific time points, and missing at other time points, but are not time invariant.   
