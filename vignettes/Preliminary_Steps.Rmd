...
### Load data 
First we load the simulated longitudinal data in long format (with missingness) that accompanies *devMSMs*. 
```{r}
...
For this example, we create 5 imputed datasets using the default random forest method and 5 iterations and assign the output to `data` for use with *devMSMs*. This code takes some time to run. 
...
Below, we specify Infancy, Toddlerhood, and Childhood epochs.  
```{r}
...
Below, we specify low economic strain at all epochs as our reference event in comparison to high levels at all epochs as well as high economic strain only in infancy.  
```{r}
reference <- c("l-l-l")
...
Optional inputs are time-varying confounders (e.g., “variable.t”), epochs, high/low cutoff values for continuous exposures, and specification of reference and comparison histories (see above).     
...
```
Here, we see summaries of the data types as well as reasonable cell counts in each of our specified histories of, for each imputed dataset. 
