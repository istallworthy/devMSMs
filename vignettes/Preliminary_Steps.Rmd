...
![Table 1. Summary of devMSMs helper functions](/Users/isabella/Documents/GitHub/devMSMsHelpers/man/figures/helper function summary table.png){width=600px}
...
* long data (with or without missing data), which can be formatted (P1a) and converted to wide data (P1b)
  
* wide data that is complete, or with missingness to be imputed using the *imputeData()* helper function (P2)
  
* imputed wide data sets saved locally, which can be read in as a list (P2b). 
...
head(data_wide,  n = c(5, 5))

```
  
Alternatively, we could start with wide data with missingness that is already formatted, and below we read in simulated wide FLP data with missingness.   
...
head(mice::complete(imputed_data, 1),  n = c(5, 5))
