---
title: "Workflow_Continuous_Exposure"
...
The `assessBalance` function saves out the following .csv and .html files into the ‘balance/prebalance’ folder: tables of balance statistics for all confounders (Table 1), tables of balance statistics for covariates that are imbalanced (Table 2), and an overall balance summary table (averaged across any imputed datasets) (Table 3). Within the ‘balance/prebalance/plots’ folder, the function outputs  .jpeg files of summary love plots depicting confounder balance for each exposure time point  ( Figure 1).  
...
We then create IPTW balancing weights using all other avaiable methods.   
...
The `assessBalance` function saves out the following .csv and .html files into the ‘balance/weighted’ folder: tables of balance statistics for all confounders (Figure x), tables of balance statistics for covariates that are imbalanced, and an overall balance summary table (averaged across any imputed datasets) ( Figure x). Within the ‘balance/weighted/plots’ folder, the function outputs .jpeg files of summary love plots depicting confounder balance for each exposure time point (Figure x). 
...
The `fitModel` function outputs a .rds file of the fitted model(s) and a .html table of model evidence (can display models for up to 12 imputed datasets) in the ‘models’ folder. Importantly, this function also outputs into the console the result of a likelihood ratio test comparing the user-specified model to a nested version of that model that omits all exposure variables to test whether exposure predicts variation in the outcome. If this test is not significant and there is no evidence that exposure predicts outcome, we do not advise proceeding to the subsequent history comparison step. Models are pooled prior to conducting the likelihood ratio test for imputed data.   
