...
There are severeal other optional fields that a user could specify in the MSM object.
...
We can also summarize the `assessBalance()` output to view the average remaining relation between covariates and exposure as well as a summary table showing the total number of imbalanced covariates at each exposure time point. We can view this at one or all time points, in one imputed dataset or averaged across them.  
```{r}
...
For `method`, provide one of the following methods for calculating balancing weights using `weightitMSM()` from the methods that have been validated for longitudinal exposures: "cbps" (Covariate Balancing Propensity Score weighting; default), “gbm” (generalized boosted model), “glm” (generalized linear model), "iptw" (inverse probability tilting) or “super” (SuperLearner via the *SuperLearner* package; Polley et al., 2013). More information can be found in the <a href="(https://ngreifer.github.io/WeightIt/reference/weightitMSM.html">*WeightIt* documentation</a>.  
...
weights.cbps <- createWeights(obj = obj, 
                             ...
                             method = method,
                             formulas = formulas,
                             maxit = 1000, # getting warning about optimization failing to converge
                             verbose = TRUE)
```
