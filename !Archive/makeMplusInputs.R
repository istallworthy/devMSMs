...
makeMplusInputs <- function(object, mplusObject) {
  ...
  if(dir.exists(paste0(home_dir, "for Mplus/original/"))==F){dir.create(paste0(home_dir, "for Mplus/original/"))}
  if(dir.exists(paste0(home_dir, "for Mplus/sensitivity checks/"))==F){dir.create(paste0(home_dir, "for Mplus/sensitivity checks/"))}

  if(dir.exists(paste0(home_dir, "for Mplus/input values/"))==F){dir.create(paste0(home_dir, "for Mplus/input values/"))}
  if(dir.exists(paste0(home_dir, "for Mplus/input values/original/"))==F){dir.create(paste0(home_dir, "for Mplus/input values/original/"))}
  if(dir.exists(paste0(home_dir, "for Mplus/input values/senstivity checks/"))==F){dir.create(paste0(home_dir, "for Mplus/input values/sensitivity checks/"))}
  ...
  for (c in 1:length(all_cutoffs)){
    ...
    for (a in seq(unlist(covariates$outcomes))){
      ...
      for (h in 1:length(exposures)){
        ...
        weights=paste0(exposure, "_", outcome, "_", cutoff, "_weight_cutoff")
        ...
        write.csv(epoch_info, paste0(home_dir, 'for Mplus/input values/',folder_label, "/", exposure, "epoch_hi_lo_values.csv"))
        ...
        constraints=noquote(paste0(paste("new ("), paste0(histories, sep="", collapse=" "), " ",
                                   ...
                                   ");", sep=" ", collapse=" "))
        write.csv(constraints, paste0(home_dir, 'for Mplus/input values/', folder_label, '/all_constraints.csv'))
        ...
        write.csv(ref_parameters, paste0(home_dir, 'for Mplus/input values/', folder_label, '/parameter_values_', exposure, '.csv'))
        ...
        colnames(q_forms)="V1"
        write.csv(forms, paste0(home_dir, 'for Mplus/input values/', folder_label, '/equations_', exposure, '.csv'))
        ...
        write.inp.file(model_input,fixPath(file.path(paste0(home_dir, "for Mplus/",folder_label, "/", exposure, "_", outcome, "_", cutoff, ".inp",sep=""))))
