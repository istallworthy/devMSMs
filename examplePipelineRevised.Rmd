...
# data_wide <- read.csv("/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/Tutorial paper/merged_tutorial_filtered.csv", header = TRUE)
# data_wide <- data_wide %>%  select(-c(contains(":")))
...
m <- 1 #provide an imputation number
method <- "pmm"; # provide an imputation method pmm, midastouch, sample, cart , rf (default)

imputed_data <- imputeData(data_wide, m, method, home_dir, exposure, outcome, para_proc = TRUE, read_imps_from_file = "no")
...
#optional list of quantiles specifying high and low cutoff values for continuous exposures; default is median
hi_lo_cut <- c(0.6, 0.3)
