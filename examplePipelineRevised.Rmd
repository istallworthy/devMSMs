...
#get helper files from github. Note: you need to install devMSMs for inspectData to work. 
...
tv_confounders <- c("SAAmylase.6","SAAmylase.15", "SAAmylase.24",
                    "MDI.6", "MDI.15",                                            
                    # "RHasSO.6", "RHasSO.15", "RHasSO.24", #removed for low variability --check this
                    "RHasSO.35", "RHasSO.58",                                         
...
data_long <- read.csv("/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/Tutorial paper/merged_tutorial_filtered.csv", 
                      header = TRUE)
...
data_long_f <- formatLongData(home_dir, data_long, exposure, exposure_time_pts, outcome, 
                            ...
                            factor_confounders = c("state","TcBlac2", "PmBlac2", "RHasSO", "PmMrSt2", "HomeOwnd"), #list factor variables here
                            save.out = TRUE) 
...
library(dplyr)
data_wide <- data_wide %>% 
  dplyr::select(-c(RHasSO.6, RHasSO.15 , RHasSO.24))
...
imputed_data <- imputeData(data_wide, m, method, home_dir, exposure, outcome, 
                           maxit = 0, para_proc = TRUE, read_imps_from_file = "no", save.out = TRUE)
...
weights.ps <- createWeights(home_dir, data, exposure, outcome, formulas, 
                            method = method, read_in_from_file = "no", verbose = TRUE, save.out = TRUE)
...
weights.gbm <- createWeights(home_dir, data, exposure, outcome, formulas, 
                             method = method, 
                             criterion = "p.max", distribution = "laplace") #can now add any input that weightitMSM() takes for "gbm
...
method <- "super"
weights.super <- createWeights(home_dir, data, exposure, outcome, formulas, method = method)
...
balance_stats.bart <- assessBalance(home_dir, data, exposure, exposure_time_pts, outcome, type, formulas, weights, 
                                    balance_thresh, imp_conf, verbose = TRUE, save.out = TRUE)
...
#manually list remaining imbalanced covariates that are time-invariant or measured at t=1 
...
Note: as of now, these will overwrite the above main analysis using assessBalance. Please run the above main analysis and then rename the output before running each sensitivity test and renaming each one successively. 
```{r}

#sensitivity tests
weights <- trim_weights.s1
final_balance_stats.s1 <- assessBalance(home_dir, data, exposure, exposure_time_pts, outcome, type, formulas, weights, 
                                        balance_thresh, imp_conf, verbose = TRUE, save.out = TRUE) 

weights <- trim_weights.s2
final_balance_stats.s2 <- assessBalance(home_dir, data, exposure, exposure_time_pts, outcome, type, formulas, weights, 
                                        balance_thresh, imp_conf, verbose = TRUE, save.out = TRUE) 
...
##### Sensitvity analyses
We recommend specifying sensitivity analyses to match the main analyses above. 
Note: as of now, these will overwrite the above main analysis using fitModel. Please run the above main analysis and then rename the output before running each sensitivity test and renaming each one successively.
```{r}
...
#all inputs
results <- compareHistories(home_dir, exposure, exposure_time_pts, outcome, model, 
...
##### Sensitvity analyses
We recommend specifying sensitivity analyses to match the main analyses above. 
Note: as of now, these will overwrite the above main analysis using compareHistories. Please run the above main analysis and then rename the output before running each sensitivity test and renaming each one successively.
```{r}

#sensitivity tests
model <- models.s1 
results.s1 <- compareHistories(home_dir, exposure, exposure_time_pts, outcome, model, 
                               epochs, hi_lo_cut, reference, comparison, mc_comp_method , dose_level, exp_lab, out_lab, colors, verbose = TRUE, save.out = TRUE) 

model <- models.s2 
results.s2 <- compareHistories(home_dir, exposure, exposure_time_pts, outcome, model, 
                               epochs, hi_lo_cut, reference, comparison, mc_comp_method , dose_level, exp_lab, out_lab, colors, verbose = TRUE, save.out = TRUE) 
