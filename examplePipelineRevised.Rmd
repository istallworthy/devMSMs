...
Choose from the following preliminary steps with the goal of assigning to 'data' one of the following forms of complete data for use in the package:
...
Data columns should be either numeric, integer, or factor form (no character variables).   
...
Users have the option of loading in data in long format. These data can be complete or have missingness. 
...
integer_covars <- c("ID", "KFASTScr", "PmEd2", "RMomAgeU", "SWghtLB", "peri_health", "caregiv_health" , "gov_assist", "B18Raw", "EARS_TJo", "MDI")
data_long_f[, integer_covars] <- as.data.frame(lapply(data_long_f[, integer_covars], as.integer))
...
After formatting their long data, users can convert their data from long to wide format (as required by the package). They may see some warnings about NA values.    
```{r}
...
Alternatively, a user could read in and format wide data (with or without missingness).   
```{r}
...
Or, read in wide data that are already formatted.  
```{r}
...
#optional
#please see the mice::mice() documentation for more optimal input to customize mice(): 
...
Alternatively, in the case of no missing data, a user could read in and format a single wide data frame of complete data for use with *devMSMs*.  
```{r}
...
Alternatively, users could impute their wide missing data using other software and read in and check imputed data files as data frames in a list for use with *devMSMs*.  
```{r}
