...
# 
# #creating long dataset for testing
# data_wide <- read.csv('/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/testing/testing data/FLP_wide_imputed.csv')
# tv <- tidyr::pivot_longer(data_wide %>% select(c(ID, tv_confounders)),
#                     cols = -c(ID), 
#                     names_to = c('.value', 'WAVE'), 
#                     names_sep = '\\.')
# 
# all_miss <- as.logical(unlist(lapply(1:nrow(tv), function(x) {
#   !all(is.na(tv[x, 3:ncol(tv)]))
# })))
# tv <- tv[all_miss, ]
# 
# ti <- data_wide %>% select(c(ID, ti_confounders))
# 
# data_long <- merge(ti, tv, by = "ID", all = TRUE)
# write.csv(data_long, '/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/testing/testing data/FLP_long_missing.csv')
# data_long = read.csv('/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/testing/testing data/FLP_long_missing.csv')
...
#all inputs
inspectData(data, home_dir, exposure, exposure_time_pts, outcome, ti_confounders, tv_confounders, # required input
...
#all inputs
full_formulas <- createFormulas(exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders, #required
                                concur_conf = concur_conf, custom = custom, #optional
                                home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

# #or minimum inputs
# full_formulas <- createFormulas(home_dir, exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders)
...
#all inputs
short_formulas <- createFormulas(exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders, #required
                                 concur_conf = concur_conf, keep_conf = keep_conf, custom = custom, #optional
                                 home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

# #or minimum inputs
# short_formulas <- createFormulas(home_,dir, exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders)
...
method <- "cbps"
weights.cbps <- createWeights(data, exposure, outcome, formulas, #required
                              method = method, read_in_from_file = FALSE,  #optional
                              home_dir = home_dir, verbose = TRUE, save.out = TRUE)  #optional

method <- "glm"
weights.glm <- createWeights(data, exposure, outcome, formulas, #required
                             method = method, read_in_from_file = FALSE, #optional
                             home_dir = home_dir, verbose = TRUE, save.out = TRUE)  #optional

method <- "gbm"
weights.gbm <- createWeights(data, exposure, outcome, formulas, #required
                             method = method, read_in_from_file = FALSE,  #optional
                             home_dir = home_dir, verbose = TRUE, save.out = TRUE)  #optional
...
method <- "bart"
weights.bart <- createWeights(data, exposure, outcome, formulas, #required
                              method = method, read_in_from_file = FALSE, #optional
                              home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

method <- "super"
weights.super <- createWeights(data, exposure, outcome, formulas, #required
                               method = method, read_in_from_file = FALSE, #optional
                               home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
balance_stats.cbps <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                    balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                    home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

weights <- weights.glm 
balance_stats.glm <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                   balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                   home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
weights <- weights.gbm
balance_stats.gbm <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                   balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                   home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

weights <- weights.bart
balance_stats.bart <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                    balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                    home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

weights <- weights.super 
balance_stats.super <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                     balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                     home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
#all inputs
balance_stats <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                               balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                               home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
#all inputs
updated_formulas <- createFormulas(exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders, bal_stats = bal_stats, #required
                                   concur_conf = concur_conf, keep_conf = keep_conf, #optional
                                   home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

#minimum inputs
# updated_formulas <- createFormulas(home_dir, exposure, exposure_time_pts, outcome, type, ti_confounders, tv_confounders,
#                                    bal_stats = bal_stats)
...
#all inputs
final_weights <- createWeights(data, exposure, outcome, formulas, #required
                               method = method, read_in_from_file = FALSE, #optional
                               home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
#all inputs
trim_weights <- trimWeights(exposure, outcome, weights, #required
...
trim_weights.s1 <- trimWeights(exposure, outcome, weights, #required
                               quantile = quantile, #optional
                               home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

quantile <- 0.98 #optional quantile of weights above which weights are trimmed (default is 0.95)
trim_weights.s2 <- trimWeights(exposure, outcome, weights, #required
                               quantile = quantile, #optional
                               home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
#all input
final_balance_stats <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                     balance_thresh = balance_thresh, imp_conf = imp_conf, #optional
                                     home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
...
final_balance_stats.s1 <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                        balance_thresh = balane_thresh, imp_conf = imp_conf, #optional
                                        home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

weights <- trim_weights.s2
final_balance_stats.s2 <- assessBalance(data, exposure, exposure_time_pts, outcome, type, formulas, weights, #required
                                        balance_thresh = balane_thresh, imp_conf = imp_conf, #optional
                                        home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
```
...
#required
model <- "m3"
models <- fitModel(data, weights, exposure, exposure_time_pts, outcome, model, #required
...
#sensitivity tests
weights <- trim_weights.s1
models.s1 <- fitModel(data, weights, exposure, exposure_time_pts, outcome, model, #required
                      family = family, link = link, int_order = int_order, covariates = covariates, epochs = epochs, #optional
                      home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional

weights <- trim_weights.s2
models.s2 <- fitModel(data, weights, exposure, exposure_time_pts, outcome, model, #required
                      family = family, link = link, int_order = int_order, covariates = covariates, epochs = epochs, #optional
                      home_dir = home_dir, verbose = TRUE, save.out = TRUE) #optional
```
...
#all inputs
results <- compareHistories(exposure, exposure_time_pts, outcome, model, #required
...
#sensitivity tests
model <- models.s1 
results.s1 <- compareHistories(exposure, exposure_time_pts, outcome, model, #required
                               epochs = epochs, hi_lo_cut = hi_lo_cut, reference = reference, comparison = comparison, #optional
                               mc_comp_method = mc_method , dose_level = dose_level, exp_lab = exp_lab, out_lab = out_lab, colors = colors, #optional
                               home_dir = home_dir, verbose = FALSE, save.out = TRUE) #optional

model <- models.s2 
results.s2 <- compareHistories(exposure, exposure_time_pts, outcome, model, #required
                               epochs = epochs, hi_lo_cut = hi_lo_cut, reference = reference, comparison = comparison, #optional
                               mc_comp_method = mc_method , dose_level = dose_level, exp_lab = exp_lab, out_lab = out_lab, colors = colors, #optional
                               home_dir = home_dir, verbose = FALSE, save.out = TRUE) #optional
