...
## Specifying Package Core Inputs
```{r}
...
## STEP P: Preliminary Steps (optional --all helper functions)

### P1. Format Data
#### P1a. Format long data
```{r}

# if you have long data with wrong time, id, and/or missing indicators
data_long <- formatLongData(home_dir, data_orig, exposure, exposure_time_pts, outcome, tv_confounders, 
                            ...
                            factor_confounders = c("state","TcBlac2", "PmBlac2", "RHasSO")) #list factor variables here
```

#### P1b. Convert data from long to wide format
```{r}
...
#### P2a. Multiply impute data using mice
```{r}
...
# data <- imputed_data
data <- complete(imputed_data, 1) #just for testing purposes

```

#### P2b. Read in imputed data saved locally
```{r}

#read in imputed csv files to list
folder <- "/Users/isabella/Library/CloudStorage/Box-Box/BSL General/MSMs/testing/imputations/" #change this to match your local folder
...
custom <- NULL
#note: below is an example of just one of the entries in the list of a custom formula --you would need to make a list that contains entries for each exposure time point that mimics the output of createFormulas. <--Run that to see what it looks like.
...
weights.super <- createWeights(home_dir, data, exposure, outcome, tv_confounders, formulas, 
                               method, read_in_from_file = "no", SL.library = SL.library, verbose = TRUE, save.out = TRUE)
