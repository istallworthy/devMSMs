...
Please reach out to Isabella Stallworthy (stallworthyisabella@gmail.com) with any questions. 
...
Collider resources? see david's stuff or ask dan 
...
forms=createForms(object, wide_long_datasets, covariates_to_include, potential_colliders, keep_covariates)
...
You can stop here if you have specific MSM models you want to fit or you can continue on to fit a standard single-outcome model.  May want to do robustness/sensitivity checks for 95th and 99th? 
```{r}
#truncate weights 
...
This code creates a template for fitting weighted more complex growth models in MPlus in the form of a .inp file. The code defaults to estimating slope (s) and quadratic (qd) growth --the user is advised to apply the appropriate transformations before establishing the appropriate functional form of the outcomes separately, and then modify these input files accordingly. 

[Meriah to add more info about what these models do]

assumes same missing indicator as original data
abridge dataset --only include necessary variables?
data must have outcomes in long form but exposures in wide form --check to see if there's a wide/long version of data saved out somewhere?
IS needs to more thoroughly automate interaction terms
```{r}
...
makeMplusInputs(object, covariates, reference, hi_cutoff=.75, lo_cutoff=.25)



#automatically run all inputs in the Mplus folder after you have manually checked them
