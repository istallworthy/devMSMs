...
data_path="/Users/isabella/Library/CloudStorage/Box-Box/BSL General/Isa/MSMs/Meriah/MSM_ALL_LONG_8.25.20.csv"
home_dir="/Users/isabella/Library/CloudStorage/Box-Box/BSL General/Isa/MSMs/Meriah/"

#individual identifier 
ID="s_id"

#time points of interest and name of time variable in long dataset
time_pts=c(6, 15, 24, 35, 58, 90, 154)
time_var="TAge"
...
exposures=c("HOMEETA1", "CTSETA1", "ESETA1")
exposure_time_pts=c(6, 15, 24, 35, 58, 90, 154)
...
exposure_epochs=data.frame(epochs=c("early", "middle", "late"), #names of epochs 
                           values=I(list(c(6,15), c(24,35), c(58, 90, 154)))) #time point(s) encompassed in each epoch that correspond to data

#outcomes of interest and outcome time point(s)
outcomes=c("CORTBASE", "sAABASE")
outcome_time_pts=c(6, 15, 24, 35, 58, 90, 154)
...
These functions requires a clean dataset in long format that contains columns for ID, time, exposure, outcome, and all potential covariate confounds as columns. Column names can include only an underscore delimeter. 
```{r}
...
MSMs focus on the covariates that are associated with both exposure and outcome and thus could act like a confounding variable. The method assumes that you have included all possible confounding variables as columns in your dataset. More specifically, a potential confound for a given exposure-outcome pair at a given time point is any covariate correlated with the exposure at that time point, lagged values of the exposure, and those correlated with the outcome. 
The user can specify any covariates to exclude ('exclude_covariates') as well as those to include ('keep_covariates'), regardless of their correlation with exposures/outcomes. Append ".x" (e.g., "INR.6) to indicate a time-varying variable at a specific time point only otherwise time-varying variables will be assumed at all time points. 
```{r}
...
exclude_covariates=c("Index1", "WAVE2", "WAVE3", "ESETA1")
...
```{r}
#list any time-varying variables that should NOT be present in the datasets because of planned missingness design with ".time format 
...
This code caps weights at a designated percentile (that the user has the option to specify with 'percentile_cutoff'), populating all weights larger than that value with that percentile value. These weights can now be used in the formal marginal structural model, or weighted model. You can stop here if you have specific MSM models you want to fit or you can continue on to fit a single-outcome model. 
```{r}
...
This code fits a series of weighted generalized linear models (using the 'survey' package) using the final truncated weights to separately examine the putative causal effects of each exposure on each (single time point) outcome, testing for the need for any covariates that were not successfully balanced. Fit is assessed using AIC. The current approach retains main effects of each exposure epoch for the purpose of examining different histories in the next step, and tests for all 2-way interactions between exposure main effects. 
Model dictionary:
