...
imputeData <- function(object, data_to_impute, read_imps_from_file="no"){
  ...
  if (read_imps_from_file=="yes"){
    ...
    # f=as.formula(paste0(outcome, " ~ ", paste(colnames(test)[grepl("ESETA1", colnames(test))], collapse=" + ")))
    # m=c(rep("norm", 2), "mean(")
    #
    # imps <- smcfcs(test, smtype="lm", m=1, smformula="y~z+x+xsq",method=c("","","norm","x^2",""))


    set.seed(123)
    test=data_to_impute_full%>%dplyr::select(outcome, contains("ESETA1"))
    #using mice to include interactions in all equations: https://uknowledge.uky.edu/cgi/viewcontent.cgi?article=1234&context=cph_etds
    ## Analysis 5 All Interations ".al"
    ini <- mice(test, maxit=0, print=FALSE)
    ...
    imp.al <- mice(data.missing, print = FALSE, m =5, maxit = 10,
                   formulas = myformulas)
    ...
    miceout <- mice::mice(data_to_impute_full, m=1, method="pmm", maxit = 5,
                          print = F)
