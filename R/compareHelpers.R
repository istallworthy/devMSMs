...
create_custom_comparisons <- function(preds, ref_vals, comp_vals, exposure) {
  ...
  if (nrow(comp_vals) > 1) {
    colnames(cus_comps) <- paste0("(", paste0(
      paste0(ref_vals, collapse = ","), ") - (",
...
add_histories <- function(p, d) {
  ...
  if (all(d$e %in% colnames(p))) {
    for (i in seq_len(nrow(p))) {
      ...
      history[i] <- as.data.frame(paste(
        ifelse(round(as.numeric(as.character(vals)), 3) ==
          round(as.numeric(as.character(d$l)), 3), "l", "h"),
        collapse = "-"
      )) # was getting different rounding values due to floaters (i think), so made char first
    }
...
add_dose <- function(p, dose_level) {
  if (length(p$history[1]) == 1 && grepl("vs", p$history[1])) {
    dose_a <- string_count(
      sapply(strsplit(p$history, "vs"), "[", 1),
      dose_level
    )
    dose_b <- string_count(
      sapply(strsplit(p$history, "vs"), "[", 2),
      dose_level
    )
...
perform_multiple_comparison_correction <- function(comps, reference, comp_histories,
  ...
  if (length(comps) > 1 || length(comps$term) > 1) {
    ...
    if (verbose) {
      cat("\n")
      cat(sprintf(
        "The user specified comparison only between %s and a single comparison, %s,
