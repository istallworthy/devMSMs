...
eval_hist <- function(data, exposure, time_pts, epochs = NULL, hi_lo_cut = NULL,
  ...
  if (is.null(epochs)) { # making epochs time pts if not specified by user
    ...
    for (e in seq_len(nrow(epochs))) {
      ...
      for (l in seq_len(length(as.numeric(unlist(epochs[e, 2]))))) {
        ...
        z <- as.data.frame(data_wide[, names(data_wide)[grepl(
          exposure,
          names(data_wide)
        )]]) # finds exposure vars
      ...
      new$history <- lapply(seq_len(nrow(new)), function(x) {
        paste(lapply(seq_len(nrow(epochs)), function(y) {
          ...
          if (new[x, y + 1] >= as.numeric(median((new[, y + 1]),
            na.rm = TRUE
          ) + 0.001)) { # added 0.001 bc marginaleffects needs different values
            return("h")
          }
          if (new[x, y + 1] <= as.numeric(median((new[, y + 1]),
            na.rm = TRUE
  ...
  if (!is.null(hi_lo_cut)) {
    cat(sprintf(
      "USER ALERT: Out of the total of %s individuals in the sample, below is the distribution of the %s (%s%%) individuals that fall into %s out of the %s the total user-defined exposure histories created from %sth and %sth percentile values for low and high levels of exposure %s, respectively, across %s. \n",
      nrow(data_wide),
      sum(his_summ$n),
    ...
    cat(sprintf(
      "USER ALERT: Out of the total of %s individuals in the sample, below is the distribution of the %s (%s%%) individuals that fall into %s out of the %s total user-defined exposure histories created from median split values for low and high levels of exposure %s, respectively, across %s. \n",
      nrow(data_wide),
      sum(his_summ$n),
      round((sum(his_summ$n) / nrow(data_wide)) * 100, 2),
      nrow(his_summ),
  ...
  if (nrow(his_summ) != length(tot_hist)) {
    warning(
      sprintf(
        "USER ALERT: There are no individuals in your sample that fall into %s exposure history/histories. You may wish to consider different high/low cutoffs (for continuous exposures), alternative epochs, or choose a different measure to avoid extrapolation.\n",
  ...
  cat("\n")
  cat(
    knitr::kable(his_summ,
      caption = sprintf(
        "Summary of user-specified exposure %s histories based on exposure main effects %s containing time points %s:",
