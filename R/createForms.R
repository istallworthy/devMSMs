...
#' @examples
createForms <- function(exposures, outcomes, time_pts, potential_colliders=NULL){
  ...
  #dterming column names
  imp_col_names=colnames(imp1_widelong)
  ...
  for (y in 1:length(exposures)){
    ...
    for (x in 1:length(time_pts)){
      ...
      if (sum(covariates %in% imp_col_names)<length(covariates)){
        print(paste0("The following lagged covariates for ", exposure, " at time ", time, " were not found in the dataset and thus skipped: ", c(covariates[!covariates %in% imp_col_names])))
      }
      ...
      print(paste0("Please inspect the weights formula below for exposure ", exposure, " at time point ", as.character(time),
