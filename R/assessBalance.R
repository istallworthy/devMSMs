...
#' @examples assessBalance(object, weights_models=list(), balance_thresh=0.12, just_made_weights="no")
assessBalance <- function (object, weights_models=list(), balance_thresh=0.12, just_made_weights="no"){
  ...
  for (f in 1:length(weights_models)){
    ...
    print(paste0("A list of any unbalanced variables for ", names(weights_models)[f] ," has now been added to the 'balance/unbalanced covariates/' folder as a .csv file"))
  ...
  for (z in seq(length(outcomes))){
    ...
    for (x in 1:length(exposures)){
      ...
      for (y in 1:length(exposure_time_pts)){
        ...
        write.csv(exposure_time_corrs, file=paste(home_dir, "balance/post-balance correlation values/all_post_balance_cors_", exposures[x],"-", outcomes[z], "_", time_pts[y], ".csv", sep=""))
        print(paste0("Check 'balance/post-balance correlation values/' folder for a csv file for ",exposures[x],"-", outcomes[z], "_", time_pts[y] ," of all correlations post-balancing and averages"))
        ...
        write.csv(sig, file=paste(home_dir, "balance/post-balance correlation values/all_sig_post_balance_cors_", exposures[x],"-", outcomes[z], "_", time_pts[y], ".csv", sep=""))
        print(paste0("USER ALERT: Check 'balance/post-balance correlation values/' folder for a csv file of all SIGNIFICANT correlations above ", as.character(balance_thresh), " for ",  exposures[x], "-", outcomes[z],"_", time_pts[y]," post-balancing and averages"))
      ...
      print(paste0("All covariates still significantly correlated with ", exposures[x], " have been saved as a csv file in 'balance/post-balance correlation values/'"))
