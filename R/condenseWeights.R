...
#'
condenseWeights <-function(ID, home_dir, m, weights_models, exposures, outcomes, time_pts){
  ...
  for (k in 1:m){
    ...
    for (z in seq(length(outcomes))){
      ...
      for (y in 1:length(exposures)){
        ...
        ggplot2::ggsave(paste("Hist_imp_", k, "_", exposure, "-", outcome, "_ALL_TIMES", ".png", sep=""), path=paste0(home_dir, "combined weights"), height=8, width=14)
        print(paste0("A histogram of weights for imputation ", k, " and exposure ", exposure, " on ", outcome, " has been saved to the 'combined weights' folder"))
        ...
        write.csv(x=as.data.frame(treat_temp_prod), file=paste(home_dir, "combined weights/Imp_", k, "_", exposure, "-", outcome, "_mean_weight.csv", sep=""))
        print(paste0("Weights  for imputation ", k, " and exposure ", exposure, " on ", outcome, " have been saved as csv files in 'combined weights'"))
      }
    }
  }
