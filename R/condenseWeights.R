...
condenseWeights <-function(object, weights_models){
    ...
    for (z in seq(length(outcomes))){
      ...
      for (y in 1:length(exposures)){
        ...
        for (x in 1:length(exposure_time_pts)){
          ...
          weights=weights_models[[paste("fit_", k, "_", exposure, "-", outcome, "-", time, sep="")]]
  ...
  for (z in seq(outcomes)){
    ...
    for (y in 1:length(exposures)){
      ...
      cat(paste0("Weights for exposure ", exposure, " on outcome ", outcome, " are now saved as a csv file in the 'final weights/values/' folder"),"\n")
    }
  }
