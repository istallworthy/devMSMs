...
compareHistories <- function(home_dir, exposure, exposure_time_pts, outcome, model, epochs = NULL, hi_lo_cut = NULL,
  ...
  ints <- gsub(
    ...
    as.character(unlist(strsplit(
      as.character(unlist(model[[1]]$terms)),
      "\\+"
    )))
  )
  ...
  if (verbose) {
    eval_hist(
      data = model[[1]]$data, exposure = exposure, epochs = epochs,
  ...
  if (is.null(epochs)) { # making epochs time pts if not specified by user
    epochs <- data.frame(
      epochs = as.character(exposure_time_pts),
  ...
  terms <- gsub(" ", "", as.character(unlist(strsplit(
    as.character(unlist(terms[3])),
    "\\+"
  ))))
    ...
    for (t in seq_len(length(eps))) {
      ...
      if (is.null(hi_lo_cut)) { # default is median split
        epoch_info$low[t] <- as.numeric(median(data[, var_name] - 0.001,
          na.rm = T
        )) # padded with -.001 bc otherwise breaks hypotheses()
  ...
  d <- data.frame(
    e = paste(epoch_info[[1]], epoch_info[[2]], sep = "."),
    l = epoch_info$low,
    h = epoch_info$high
  )
  ...
  if (!is.null(reference)) {
    ...
    if (sum(exposure_levels %in% reference) != length(reference)) {
      stop(
        paste0("If you wish to conduct custom comparisons, please select a valid reference history from the following list:
                  ", paste(
          apply(perm2(nrow(epochs), c("l", "h")), 1,
            paste,
            sep = ",", collapse = "-"
          ),
          sep = ", ",
    ...
    if (is.null(comparison)) {
      stop(
        paste0(
          "If you wish to conduct custom comparisons, please specify at least one valid comparison history from the following list:
                  ", paste0(
            apply(perm2(nrow(epochs), c("l", "h")), 1,
              paste,
              sep = ",", collapse = "-"
  ...
  if (!is.null(comparison)) {
    ...
    if (sum(exposure_levels %in% comparison) != length(comparison)) {
      stop(paste0(
        "If you wish to specify comparison(s), please provide at least one comparison history from the following list ",
        paste0(
          apply(perm2(nrow(epochs), c("l", "h")), 1,
            paste,
            sep = "", collapse = "-"
  ...
  if (is.null(reference) && is.null(comp_histories)) {
    ...
    comps <- create_custom_contrasts(
      d, reference, comp_histories,
      exposure, preds
    )
  }
  ...
  if (is.null(names(model))) { # imputed data
    ...
    tidy.pred_custom <<- function(x, ...) {
      ...
      out$term <- do.call(sprintf, c(
        paste0(paste(names(args),
          collapse = " = %s, "
        ), " = %s"),
    ...
    if (save.out) {
      ...
      outfile <- file.path(
        home_dir, "histories",
        sprintf(
          "%s-%s_pooled_estimated_means_hi_lo=%s.html",
          exposure, outcome,
    ...
    comps_pool <- perform_multiple_comparison_correction(
      comps_pool,
      reference,
      comp_histories,
      mc_comp_method,
    ...
    comps_pool$term <- unlist(lapply(1:length(comps_pool$term), function(x) {
      ...
      a <- paste0("(", paste(
        round(as.numeric(as.character(unlist(strsplit(gsub(
          "[^0-9.-]", " ",
          sapply(strsplit(x, "\\ - "), "[", 1)
        ), " "))[
          !is.na(as.numeric(unlist(strsplit(gsub("[^0-9.-]", " ", sapply(strsplit(x, "\\ - "), "[", 1)), " "))))
        ])), 4),
        collapse = ", ", sep = ", "
      ), ")")
      b <- paste0("(", paste(
        round(as.numeric(as.character(unlist(strsplit(gsub(
          ...
          !is.na(as.numeric(unlist(strsplit(gsub(
            "[^0-9.-]", " ",
            sapply(strsplit(x, "\\ - "), "[", 1)
          ), " "))))
        ])), 4),
    ...
    if (save.out) {
      ...
      outfile <- file.path(
        home_dir, "histories",
        sprintf(
          "%s-%s_pooled_comparisons_hi_lo=%s.html",
          exposure, outcome,
          paste(hi_lo_cut, collapse = "_")
        )
      )
    ...
    if (save.out) {
      ...
      outfile <- file.path(
        home_dir, "histories",
        sprintf(
          "%s-%s_estimated_means_hi_lo=%s.html",
          exposure, outcome,
          paste(hi_lo_cut, collapse = "_")
        )
      )
    ...
    comps <- perform_multiple_comparison_correction(
      comps,
      reference,
      comp_histories,
      mc_comp_method,
  ...
  if (length(colors) > 1) { # If user input a list of colors
    p <- ggplot2::ggplot(data = comparisons, ggplot2::aes(
      x = .data$estimate,
      y = .data$history,
      color = .data$dose
    )) +
      ...
      ggplot2::scale_y_discrete(
        limits = c(as.character(comparisons$history)),
        expand = c(0, 0.2)
      ) +
      ...
      ggplot2::xlim(
        min(comparisons$low_ci) - 1 * sd(comparisons$low_ci),
